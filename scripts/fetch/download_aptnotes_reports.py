#!/usr/bin/env python3
"""Download APT reports from Box.com using aptnotes/data metadata.

This script downloads PDF reports from Box.com based on the APTnotes.csv
metadata file. Reports are organized by year and verified using SHA-1 hashes.

Dependencies:
    - beautifulsoup4
    - lxml
    - python-magic-bin (for Windows) or python-magic (for Unix)
    - requests

Output:
    - data/reports/aptnotes_pdfs/YYYY/filename.pdf
"""
...
def get_download_url(page: str) -> str:
    """Parse Box preview page to extract download URL.

    Args:
        page: HTML content of Box preview page
...
def download_report(report: dict, output_base: Path) -> bool:
    """Download a single report from Box.com.

    Args:
        report: Dictionary with report metadata
...
def load_reports_from_csv(csv_path: Path) -> list:
    """Load report metadata from CSV file.

    Args:
        csv_path: Path to APTnotes.csv
...
def main():
    ...
    csv_path = Path('../../data/reports/aptnotes_data/APTnotes.csv')
    output_base = Path('../../data/reports/aptnotes_pdfs')
    ...
    for i, report in enumerate(reports, 1):
        ...
        if report_already_downloaded(output_base / str(report['Year']) / report['Filename']):
            skip_count += 1
            continue

        if download_report(report, output_base):
            success_count += 1
        else:
            fail_count += 1
    ...
    logger.info(f"Total processed: {len(reports)}")
